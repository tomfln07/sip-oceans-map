...
import "./style.css"
import world from "../../assets/wrld.jpg"
...
export function Map() {
	...
	useEffect(() => {
		const mouse_down = (e) => {
			if (e.button == "0") {
				mouse_pos_origin.current = [e.screenX, e.screenY];
	...
	const track_mouse = (e) => {
		...
		const delta_x = mouse_pos_origin.current[0] - e.screenX;
	...
	const update_world_img_pos = (pos) => {
		if (world_img) {
			world_img.current.style.top = `${-pos[1]}px`;
			world_img.current.style.left = `${-pos[0]}px`;
		}
	}
